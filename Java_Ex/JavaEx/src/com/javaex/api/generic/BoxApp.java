...
public class BoxApp {
	...
	public static void main(String[] args) {
		...
		//	-> Object를 처리할 수 있는 객체를 만들면 모든 객체를 처리할 수 있는 만능 클래스가 된다.
		...
		System.out.println((Integer)objBox.getContent());
		//	문제점
		//	1. setter에서 전달되는 매개변수의 타입을 제한할 수 없다. 
		//	2. 데이터를 받아올 때, 캐스팅이 필요 Object -> 실제 클래스로의 변환
		
		String obj2 = (String)objBox.getContent();	//	캐스팅을 잘못하면 오류(ClassCastException)
		System.out.println(obj2);
		
		//	Generic Box
		//	내부 데이터 타입을 외부에서 객체화 시점에서 결정
		GenericBox genericStringBox = new GenericBox();
...
//		genericStrBox.setContent(2024);	//	컴파일 시점에서 내부 입력 데이터 체크
		...
		//	내부 저장 데이터가 지정되어 있기 때문에 캐스팅이 불필요.
